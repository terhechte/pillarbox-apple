...
final class AssetableTests: TestCase {
    func testPlayerItemsWithoutCurrentItem() {
        ...
        let result = AVPlayerItem.playerItems(for: currentAssets, replacing: previousAssets, currentItem: nil, length: 10)
    ...
    func testPlayerItemsWithPreservedCurrentItem() {
        ...
        let result = AVPlayerItem.playerItems(for: currentAssets, replacing: previousAssets, currentItem: currentItem, length: 10)
        let expected = [
            currentItemAsset,
            .loading.withId(UUID("B")),
            .loading.withId(UUID("C"))
    ...
    func testPlayerItemsWithUnknownCurrentItem() {
        ...
        let result = AVPlayerItem.playerItems(for: currentAssets, replacing: previousAssets, currentItem: unknownItem, length: 10)
        expect(result.count).to(equal(currentAssets.count))
        expect(zip(result, currentAssets)).to(allPass { item, asset in
            asset.matches(item)
        })
